...
export default function SettingsPage() {
  ...
  const handleAddKPI = async () => {
    ...
    try {
      ...
      if (data) {
        setKpis([...kpis, data]);
        setNewKPI({ name: '', description: '', category: '', data_type: 'number' });
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow p-6">
        {isLoading ? (
            ...
            {touchpoints.map((tp) => (
              <div key={tp.id} className="flex items-center justify-between border border-gray-200 rounded-lg p-4">
                <div>
                  ...
                  {!tp.visible && <p className="text-xs text-gray-500">Hidden</p>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            ...
            {tags.map((tag) => (
              <div key={tag.id} className="flex items-center justify-between border border-gray-200 rounded-lg p-4">
