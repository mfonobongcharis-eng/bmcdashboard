    ...
    const loadReports = async () => {
      ...
      try {
        ...
        if (recentReports.length > 0) {
          setSelectedReport(recentReports[0]);
        }
      } catch (err) {
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="grid grid-cols-4 gap-4 mt-6">
                    <div className="border border-gray-200 rounded-lg p-3 text-center">
                      ...
                      <p className="text-2xl font-bold">{selectedReport.department_summary.tasksCompleted}</p>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-3 text-center">
                      <p className="text-sm text-gray-600">Scheduled</p>
                      <p className="text-2xl font-bold">{selectedReport.department_summary.tasksScheduled}</p>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-3 text-center">
                      <p className="text-sm text-gray-600">Overdue</p>
                      <p className="text-2xl font-bold text-red-600">{selectedReport.department_summary.tasksOverdue}</p>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-3 text-center">
                      <p className="text-sm text-gray-600">Active Projects</p>
                      <p className="text-2xl font-bold">{selectedReport.department_summary.activeProjects.length}</p>
                    </div>
                  </div>
                </div>
                      ...
                      {selectedReport.department_summary.activeProjects.map((project: any) => (
                        <div key={project.name} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <p className="text-sm text-gray-600">{project.percentComplete}%</p>
                          </div>
                    ...
                    <table className="w-full text-sm">
                      <thead className="border-b border-gray-200">
                        <tr>
                          ...
                          <th className="text-left py-2 text-gray-700">Completed</th>
                          <th className="text-left py-2 text-gray-700">Status</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-200">
                        {Object.entries(selectedReport.individual_breakdown).map(([userId, member]: [string, any]) => (
                            ...
                            <td className="py-3">
                              <span
                                className={`px-2 py-1 rounded text-xs font-medium ${
                                  member.onTrackStatus === 'on-track'
                                    ? 'bg-green-100 text-green-800'
                                    : member.onTrackStatus === 'at-risk'
                                    ? 'bg-yellow-100 text-yellow-800'
                                    : 'bg-red-100 text-red-800'
                                }`}
                              >
                                {member.onTrackStatus}
                              </span>
                            </td>
                          </tr>
                        ))}
