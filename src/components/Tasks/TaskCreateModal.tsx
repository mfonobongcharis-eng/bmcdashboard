...
export default function TaskCreateModal({ isOpen, onClose }: TaskCreateModalProps) {
  ...
  // Auto-suggest project on title change
  useEffect(() => {
    if (formData.title.length > 3) {
      const suggest = async () => {
        try {
          ...
          console.error('Project suggestion failed:', err);
        }
      };
      suggest();
    }
  ...
  // Auto-suggest KPIs on step 4
  useEffect(() => {
    if (step === 4 && formData.title && kpiOptions.length > 0) {
      const suggestKPIs = async () => {
        setIsLoadingKPIs(true);
        try {
          const results = await suggestionService.suggestKPIs(
            formData.title,
            formData.description,
            projects.find((p) => p.id === formData.project_id) || undefined,
            formData.touchpoint_id,
            formData.lane,
            kpiOptions
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createTask({
        ...
        lane: formData.lane,
        touchpoint_id: formData.touchpoint_id,
        project_id: formData.project_id,
        due_date: formData.due_date,
        start_date: formData.start_date,
        priority: formData.priority,
        tags: formData.tags,
        kpi_ids: formData.kpi_ids,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full">
        ...
        <div className="h-1 bg-gray-200">
          <div className="h-full bg-blue-600 transition-all" style={{ width: `${(step / 5) * 100}%` }}></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-8 py-6 space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => { setFormData({ ...formData, title: e.target.value }); setSuggestedProject(null); }}
          ...
          {/* Step 2: Lane, Touchpoint, Project */}
          {step === 2 && (
            <div className="space-y-4">
              <div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g., Social Media, Email, Blog"
                  value={formData.touchpoint_id}
                  onChange={(e) => setFormData({ ...formData, touchpoint_id: e.target.value })}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                />
              </div>
              <div>
                ...
                {suggestedProject && !formData.project_id && (
                  <button
                    type="button"
                    onClick={() => setFormData({ ...formData, project_id: suggestedProject.projectId })}
                  ...
                  <div className="space-y-2">
                    {suggestedKPIs.length > 0 && (
                      <div className="mb-3 p-2 bg-blue-50 rounded text-sm text-blue-700">
                        {suggestedKPIs.length} suggested KPIs
                      </div>
                    )}
                    {kpiOptions.map((kpi) => (
                      <label key={kpi.id} className="flex items-center">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setFormData({ ...formData, kpi_ids: [...formData.kpi_ids, kpi.id] });
                            } else {
                              setFormData({ ...formData, kpi_ids: formData.kpi_ids.filter((id) => id !== kpi.id) });
                            }
                          }}
                          className="mr-2"
                        />
