...
export const projectService = {
  ...
  async createProject(
    ...
    // Log to audit trail
    await auditService.logAction(userId, 'created', 'project', data.id, {
      name: data.name,
      owner_id: input.owner_id,
    });
  ...
  /**
   * Delete project (hard delete - admin only)
   */
  async deleteProject(id: string, userId: string): Promise<void> {
    ...
    await auditService.logAction(userId, 'deleted', 'project', id, {
      name: project.name,
    });
  },
  ...
  async getProjectsByLane(lane: Lane): Promise<Project[]> {
    ...
    if (error) {
      console.error('Failed to fetch projects by lane:', error);
      return [];
    }
  ...
  /**
   * Get predefined projects (seeded at launch)
   * Mfon to provide the list
   */
  async getPredefinedProjects(): Promise<Project[]> {
    const predefinedNames = [
      'Medusa Loan Campaign',
      'IWD 2026',
      'Agency Banking Launch',
      // Add more as provided by Mfon
    ];

    const { data, error } = await supabase
      .from('projects')
      .select('*')
      .in('name', predefinedNames)
      .eq('is_archived', false);

    if (error) {
      console.error('Failed to fetch predefined projects:', error);
      return [];
    }
