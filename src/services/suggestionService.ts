...
import type { KPI, Project, KPISuggestion, Lane } from '../types';
...
export const suggestKPIsWithClaude = async (
  ...
  try {
    let touchpointName = '';
    if (touchpointId) {
      const { data } = await supabase.from('touchpoints').select('name').eq('id', touchpointId).single();
...
Do not include explanations, asterisks, or any other text.`;
    ...
    const claudePromise = (async () => {
      ...
      const suggestedNames = responseText.split(',').map((n: string) => n.trim()).filter((n: string) => n.length > 0);

      const suggestions = suggestedNames
        .map((name: string, index: number) => {
          const kpi = kpiDefinitions.find((k) => k.name.toLowerCase() === name.toLowerCase());
          if (!kpi) return null;
          return { kpiId: kpi.id, name: kpi.name, confidence: Math.max(95 - index * 5, 70), reason: `Matched by Claude semantic analysis` } as KPISuggestion;
        })
...
const categoryKeywords: Record<string, string[]> = {
  'Reach': ['instagram', 'linkedin', 'facebook', 'tiktok', 'post', 'social', 'publish', 'content', 'blog', 'article'],
  'Engagement': ['carousel', 'video', 'reel', 'interactive', 'engagement', 'click', 'share', 'like', 'comment'],
  'Email Open Rate': ['email', 'newsletter', 'send', 'campaign', 'mailchimp', 'mailgun', 'sendgrid'],
  'Lead Generation': ['lead', 'merchant', 'activation', 'conversion', 'form', 'signup', 'webinar', 'contact'],
  'Brand Awareness': ['brand', 'thought leadership', 'awareness', 'blog', 'article', 'press', 'media'],
...
export const suggestKPIsWithRules = (
  ...
  const text = `${taskTitle} ${taskDescription || ''} ${project?.name || ''} ${lane}`.toLowerCase();
  const matches: { name: string; score: number }[] = [];

  for (const [kpiName, keywords] of Object.entries(categoryKeywords)) {
    const matchCount = keywords.filter((k) => text.includes(k)).length;
    if (matchCount > 0) matches.push({ name: kpiName, score: matchCount });
  }

  if (lane === 'medusa') {
    if (!matches.find((m) => m.name === 'Product Support')) matches.push({ name: 'Product Support', score: 1 });
  }
  ...
  return topMatches
    .map((match) => {
      const kpi = kpiDefinitions.find((k) => k.name.toLowerCase() === match.name.toLowerCase());
      if (!kpi) return null;
      return { kpiId: kpi.id, name: kpi.name, confidence: Math.max(80 - matches.indexOf(match) * 10, 50), reason: 'Matched by rule-based keyword analysis (fallback)' } as KPISuggestion;
    })
    .filter((s): s is KPISuggestion => s !== null);
};
...
export const suggestProject = async (
  ...
  const { data: projects, error } = await supabase.from('projects').select('*').eq('is_archived', false);
  ...
  const scored = projects
    .map((project) => {
      ...
      return { ...project, confidence: Math.round(similarity * 100) };
    })
  ...
  return { projectId: topMatch.id, name: topMatch.name, confidence: topMatch.confidence };
};
